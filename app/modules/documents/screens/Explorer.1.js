...
class Explorer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { needReload, expires_date } = nextProps;

    const { username, password, sid, valid, login, renewSid } = nextProps;

    if (username && username !== that.state.username) {
      console.log('Account has been changed and the data source will be refreshed.')
      that.setState({
        username,
        password,
      }, () => {
  ...
  previewDocument = (item: any) => {
    ...
    DocumentService.downloadToCacheDirectory(sid, item, that.updateProgress, that.resetDownloadTask)
      .then(man => {
        ...
        that.downloadManger.onProgress = that.updateProgress;
